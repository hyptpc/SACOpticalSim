...
StackingAction::StackingAction()
    : G4UserStackingAction(), fScintillationAll(0), fCerenkovAll(0), fCerenkovAerogel(0), fCerenkovDelta(0)
{
}
...
G4ClassificationOfNewTrack StackingAction::ClassifyNewTrack(const G4Track* aTrack)
{
    if (aTrack->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()) { // particle is optical photon
        if (aTrack->GetParentID() > 0) {                                         // particle is secondary
            if (aTrack->GetCreatorProcess()->GetProcessName() == "Scintillation") ++fScintillationAll;
            else if (aTrack->GetCreatorProcess()->GetProcessName() == "Cerenkov") {
                ++fCerenkovAll;

                if (aTrack->GetParentID() > 1) ++fCerenkovDelta;
...
void StackingAction::NewStage()
{
    ...
    // 	 << fCerenkovAerogel << G4endl;
    gAnaMan.SetNumOfCerenkovAll(fCerenkovAll);
    gAnaMan.SetNumOfCerenkovAerogel(fCerenkovAerogel);
    gAnaMan.SetNumOfCerenkovDelta(fCerenkovDelta);
}
